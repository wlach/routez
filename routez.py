...
__doc__ = """\
The main backend code for the routez.ca web interface.
"""
...
class Main(Component):
    ...
    def lib(self, request, path):
        ...
        # This is really only useful when running standalone, as with
        # normal setups, this data should be served by a more poweful web
        # server, with caching and load balancing.
...
def FindDefaultFileDir():
    """Return the path of the directory containing the static files. By
    default the directory is called 'web_content', just off the current
    working directory."""
...
class Log:
    """File like for writes with auto flush after each write to ensure that
    everything is logged, even during an unexpected exit."""
    def __init__(self, f):
        self.f = f
    def write(self, s):
        self.f.write(s)
